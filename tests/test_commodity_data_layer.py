...
class TestAkshareIntegration:
    ...
    def test_get_futures_news_financial_已实现(self, akshare_mod, mock_ak):
        """financial(IF/IH/国债等)现在通过 QVIX/国债/SHIBOR/LPR 合成 4 类卡片"""
        provider = akshare_mod.AkshareFuturesProvider()
        provider._ak = mock_ak
        # 把全部 6 个接口返回值都设为空 df,期望返回 []
        items = asyncio.run(provider.get_futures_news(category="financial"))
        # 因为 mock 给的是 empty_df (col=[1,2,3]),字段名都不匹配,所以会跳过
        assert items == []
        # shmet 不被调用
    ...
    def test_get_futures_news_chemical_油价卡片(self, akshare_mod, mock_ak):
        """chemical → 调用 energy_oil_hist 合成汽柴油价卡片"""
        provider = akshare_mod.AkshareFuturesProvider()
        provider._ak = mock_ak
        provider._ak.energy_oil_hist.return_value = pd.DataFrame({
            "调整日期": ["2024-09-04", "2024-08-22"],
            "汽油价格": [8050, 8030],
            "柴油价格": [7080, 7070],
            "汽油涨跌": [-80, -50],
            "柴油涨跌": [-75, -50],
        ...
        items = asyncio.run(provider.get_futures_news(category="chemical", limit=10))

        # 不走 shmet
        mock_ak.futures_news_shmet.assert_not_called()
        # 必须调用 energy_oil_hist
        mock_ak.energy_oil_hist.assert_called_once()
        # 至少一条卡片
        assert len(items) > 0
        assert items[0]["source"] == "akshare_synth"
        assert items[0]["category"] == "chemical"
        assert items[0]["metal"] == "能化"
        assert "汽柴油调价" in items[0]["title"]
        assert "汽油" in items[0]["content"]

    def test_get_futures_news_energy_油价_电厂日耗(self, akshare_mod, mock_ak):
        """energy → 油价 + 沿海六大电日耗"""
        provider = akshare_mod.AkshareFuturesProvider()
        provider._ak = mock_ak
        provider._ak.energy_oil_hist.return_value = pd.DataFrame({
        ...
        provider._ak.macro_china_daily_energy.return_value = pd.DataFrame({
            "日期": ["2024-09-04"],
            "沿海六大电库存": [1250.5],
            "日耗": [85.4],
            "存煤可用天数": [14.6],
        ...
        items = asyncio.run(provider.get_futures_news(category="energy"))

        mock_ak.energy_oil_hist.assert_called_once()
        mock_ak.macro_china_daily_energy.assert_called_once()
        # 至少含一条油价卡片 + 一条电厂卡片
        titles = {it["title"] for it in items}
        assert any(t.startswith("汽柴油调价-") for t in titles)
        assert any(t.startswith("电厂日耗煤-") for t in titles)
        # category / label 已被补
        for it in items:
            assert it["category"] == "energy"
            assert it["metal"] == "能源"
            assert it["source"] == "akshare_synth"

    def test_get_futures_news_agricultural_批发价_猪_糖(self, akshare_mod, mock_ak):
        """agricultural → 批发价指数 + 生猪 + 食糖"""
        provider = akshare_mod.AkshareFuturesProvider()
        provider._ak = mock_ak
        provider._ak.macro_china_agricultural_product.return_value = pd.DataFrame({
            "日期": ["2024-09-04"],
            "最新值": [125.4],
            "涨跌幅": [-0.3],
            "近1年涨跌幅": [5.1],
        })
        provider._ak.futures_hog_supply.return_value = pd.DataFrame({
            "date": ["2024-09-04"],
            "value": [26.5],
        })
        provider._ak.index_hog_spot_price.return_value = pd.DataFrame({
            "日期": ["2024-09-04"],
            "指数": [105.6],
            "4个月均线": [102.3],
            "成交均价": [15.4],
        })
        provider._ak.index_outer_quote_sugar_msweet.return_value = pd.DataFrame({
            "日期": ["2024-09-04"],
            "巴西糖进口成本": [4500],
            "泰国糖进口利润空间": [200],
            "日照现货价": [6000],
        ...
        items = asyncio.run(provider.get_futures_news(category="agricultural", limit=30))

        # 四个接口都应被调用
        mock_ak.macro_china_agricultural_product.assert_called_once()
        mock_ak.futures_hog_supply.assert_called_once_with(symbol="猪肉批发价")
        mock_ak.index_hog_spot_price.assert_called_once()
        mock_ak.index_outer_quote_sugar_msweet.assert_called_once()

        # 4 类标题应都有
        titles = [it["title"] for it in items]
        assert any("农产品批发价200指数" in t for t in titles)
        assert any("猪肉批发价日报" in t for t in titles)
        assert any("生猪市场行情指数" in t for t in titles)
        assert any("食糖进口日报" in t for t in titles)
        # category 标注
        for it in items:
            assert it["category"] == "agricultural"
            assert it["metal"] == "农产品"
            assert it["source"] == "akshare_synth"

    def test_get_futures_news_合成器_接口抛错_不污染(self, akshare_mod, mock_ak):
        """AKShare 接口抛异常时,合成器应静默吞咽,返回空列表"""
        provider = akshare_mod.AkshareFuturesProvider()
        provider._ak = mock_ak
        mock_ak.energy_oil_hist.side_effect = Exception("网络超时")

        items = asyncio.run(provider.get_futures_news(category="chemical"))
        # 单个接口失败不影响方法返回
        assert items == [] or all("汽柴油调价" not in it["title"] for it in items)
        mock_ak.futures_news_shmet.assert_not_called()

    # ----- financial 合成器测试 -----

    def test_get_futures_news_financial_全6接口调用(self, akshare_mod, mock_ak):
        """financial category 应尝试调用 6 个宏观接口(任一字段不匹配则跳过)"""
        provider = akshare_mod.AkshareFuturesProvider()
        provider._ak = mock_ak

        items = asyncio.run(provider.get_futures_news(category="financial", limit=20))

        # 6 个接口都应被尝试调用
        mock_ak.index_option_300index_qvix.assert_called_once()
        mock_ak.index_option_300etf_qvix.assert_called_once()
        mock_ak.index_option_1000index_qvix.assert_called_once()
        mock_ak.bond_zh_us_rate.assert_called_once()
        mock_ak.macro_china_shibor_all.assert_called_once()
        mock_ak.macro_china_lpr.assert_called_once()
        # mock 给的是 col=[1,2,3] df,字段不匹配 → 返回空
        assert items == []

    def test_get_futures_news_financial_QVIX卡片生成(self, akshare_mod, mock_ak):
        """QVIX 三指数 → 三种 QVIX 卡片"""
        provider = akshare_mod.AkshareFuturesProvider()
        provider._ak = mock_ak
        ...
        qvix_df = pd.DataFrame({
            ...
            "high": [27.0, 30.0],
            "low": [23.0, 25.0],
            "close": [26.5, 32.4],   # 第二个 > 30 → negative
        })
        mock_ak.index_option_300index_qvix.return_value = qvix_df
        mock_ak.index_option_300etf_qvix.return_value = qvix_df
        mock_ak.index_option_1000index_qvix.return_value = qvix_df

        items = asyncio.run(provider.get_futures_news(category="financial"))

        # 找到 QVIX 标题
        titles = [it["title"] for it in items]
        assert any("中证300股指 QVIX" in t for t in titles)
        assert any("300 ETF 期权 QVIX" in t for t in titles)
        assert any("中证1000股指 QVIX" in t for t in titles)
        # QVIX > 30 → negative (close=32.4)
        assert any(it["sentiment"] == "negative" for it in items)
        # 卡片来源是 akshare_synth
        assert all(it["source"] == "akshare_synth" for it in items)
        assert all(it["category"] == "financial" for it in items)

    def test_get_futures_news_financial_国债收益率卡片(self, akshare_mod, mock_ak):
        """中美国债 → 含利差卡片"""
        provider = akshare_mod.AkshareFuturesProvider()
        provider._ak = mock_ak
        ...
        gov = [it for it in items if "中美国债收益率" in it["title"]]
        assert len(gov) == 1
        c = gov[0]["content"]
        assert "中10Y 2.15%" in c
        assert "美10Y 4.2%" in c
        assert "中10-2Y 25.0bp" in c
        assert "适用 TL/T/TF/TS" in c

    def test_get_futures_news_financial_SHIBOR_LPR卡片(self, akshare_mod, mock_ak):
        """SHIBOR 8 品种 + LPR 1Y/5Y"""
        provider = akshare_mod.AkshareFuturesProvider()
        provider._ak = mock_ak
        ...
        shibor = [it for it in items if "SHIBOR" in it["title"]]
        lpr = [it for it in items if "LPR" in it["title"]]
        assert len(shibor) == 1
        assert len(lpr) == 1
        assert "O/N 1.5%" in shibor[0]["content"]
        assert "LPR 1Y 3.35%" in lpr[0]["content"]
        # 利率通常是 neutral
        assert lpr[0]["sentiment"] == "neutral"

    def test_get_futures_news_financial_QVIX恐慌阈值(self, akshare_mod, mock_ak):
        """QVIX 阈值:>30 恐慌,>25 偏紧,<20 偏松,<22 平静"""
        provider = akshare_mod.AkshareFuturesProvider()
        provider._ak = mock_ak
        # 给 4 个不同 close 值测试 4 个区间
        mock_ak.index_option_300index_qvix.return_value = pd.DataFrame({
        ...
        mock_ak.index_option_300etf_qvix.return_value = pd.DataFrame(
            columns=["date", "open", "high", "low", "close"]
        )
        mock_ak.index_option_1000index_qvix.return_value = pd.DataFrame(
            columns=["date", "open", "high", "low", "close"]
        )

        items = asyncio.run(
            provider.get_futures_news(category="financial", limit=20)
        )

        # 找到 4 个 QVIX 卡片
        qvix_items = [
            it for it in items if "中证300股指 QVIX" in it["title"]
        ]
        assert len(qvix_items) == 4
        sentiments = {it["published_at"]: it["sentiment"] for it in qvix_items}
        # close=15 → positive(平静), 20 → positive(平静), 26 → negative(偏紧), 32 → negative(恐慌)
        assert sentiments["2024-09-01"] in ("positive", "neutral")
        assert sentiments["2024-09-02"] == "negative"
        assert sentiments["2024-09-03"] == "negative"
        assert sentiments["2024-09-04"] == "positive"

    def test_get_futures_news_financial_数据全NaN被跳过(self, akshare_mod, mock_ak):
        """QVIX 数据全 NaN → 该数据源被跳过,不影响其它源"""
        provider = akshare_mod.AkshareFuturesProvider()
        provider._ak = mock_ak
        # 全 NaN df
        nan_df = pd.DataFrame({
            "date": pd.to_datetime(["2024-09-04"]).date,
            "open": [float("nan")],
            "high": [float("nan")],
            "low": [float("nan")],
            "close": [float("nan")],
        })
        mock_ak.index_option_300index_qvix.return_value = nan_df

        items = asyncio.run(provider.get_futures_news(category="financial"))
        # QVIX 全 NaN 不产出卡片
        qvix_items = [it for it in items if "QVIX" in it["title"]]
        assert len(qvix_items) == 0
