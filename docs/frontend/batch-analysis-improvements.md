...
#### 修改前
```typescript
ElMessage.success(`批量分析任务已提交，共${total_tasks}只期货品种`)
...
**问题**：前端限制为100只期货品种，但后端限制为10只，不一致。
...
if (stockCodes.value.length > 100) {
  ElMessage.warning('单次批量分析最多支持100只期货品种')
  return
}
```
...
if (stockCodes.value.length > 10) {
  ElMessage.warning('单次批量分析最多支持10只期货品种，请减少期货品种数量')
  return
}
```
