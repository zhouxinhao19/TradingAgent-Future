...
### 前端
- `frontend/src/views/Favorites/index.vue` - 自选品种页面（已修改）
- `frontend/src/api/favorites.ts` - 自选品种 API（保留字段定义，但前端不再使用）

### 后端（保留不变）
- `app/routers/favorites.py` - 自选品种路由
- `app/models/user.py` - 用户模型
- `app/services/favorites_service.py` - 自选品种服务
