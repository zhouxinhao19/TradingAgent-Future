...
### 原始问题
前端在短时间内发起了大量重复的 `quote` 请求（例如：同一个品种代码的多个并发请求），导致：
...
1. 对于同一个品种代码，同一时间只有一个实际的 API 调用
...
#### 场景2：3个不同期货品种，每个5个并发请求
```
时间线：
T0: 15个请求同时到达（00700×5, 00941×5, 01810×5）
T1: 每个期货品种的第1个请求获得各自的锁，开始调用API
    其他请求等待各自期货品种的锁
T2: 3个API调用并行执行（不互相阻塞）
T3: 3个请求完成，保存到缓存，释放锁
T4: 其他12个请求从缓存获取数据

结果：只有3次API调用（每品种票1次），其他12个请求从缓存获取
```

## 优势

### 1. 防止API限速
- 避免短时间内对同一期货品种的重复API调用
- 降低触发限速保护的风险
