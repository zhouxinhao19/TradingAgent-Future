...
## 1. 目标与范围
- 目标：提供基于 A 股日线数据的最小可用选品种系统，支持条件筛选、排序、分页、模板保存、导出。
- 范围：
...
不在 P0：分钟级、复杂回测、行业中性化、事件/情绪、AI 选品种（归入 P1 PoC）。
...
- 第 3 周：
  - 压测与优化、文档与示例、验收
- P1（预研并计划落地）：
  - AI 选品种：/api/screening/ai（NL→DSL），few-shot 模板
  - 简单回测（TopN/持有期）与策略评分
...
## 13. 变更记录
- 2025-08-22: 初版（加入 KDJ、复权切换、AI 选品种 P1 预留）
