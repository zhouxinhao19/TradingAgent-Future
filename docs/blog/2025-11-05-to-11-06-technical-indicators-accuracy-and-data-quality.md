...
# 3. 还原配置文件
Copy-Item .env.backup_20251106 .env -Force

# 4. 还原 MongoDB 数据（如果需要）
...
# 2. 检出旧版本代码（如果使用 Git）
git checkout v0.1.13  # 或具体的版本标签

# 3. 还原配置文件
cp .env.backup_20251106 .env

# 4. 拉取旧版本镜像（如果使用 Docker Hub）
# 编辑 docker-compose.hub.nginx.yml，指定旧版本镜像标签
# image: hsliup/tradingagents-backend:v0.1.13
# image: hsliup/tradingagents-frontend:v0.1.13
...
- [绿色版备份还原升级指南](green-version-backup-restore-upgrade.md) - MongoDB 数据备份详细指南
- [快速开始.md](../../release/TradingAgentsCN-portable/快速开始.md) - 绿色版快速启动指南
- [停止服务说明.md](../../release/TradingAgentsCN-portable/停止服务说明.md) - 停止服务脚本使用说明
- [端口配置说明.md](../../release/TradingAgentsCN-portable/端口配置说明.md) - 端口冲突解决方案
- [DOCKER_QUICKSTART.md](../../DOCKER_QUICKSTART.md) - Docker 快速启动指南
# 进入 MongoDB 容器备份数据
docker exec tradingagents-mongodb mongodump --db tradingagents --out /data/db/backup_$(date +%Y%m%d)

# 或复制备份到宿主机
docker cp tradingagents-mongodb:/data/db ./mongodb_backup_$(date +%Y%m%d)
```
...
```bash
# 查看技术指标日志
docker-compose logs backend | grep "技术指标详情"
```
...
```bash
# 进入 MongoDB 容器
docker exec -it tradingagents-mongodb mongodump --db tradingagents --out /backup

# 复制备份到宿主机
docker cp tradingagents-mongodb:/backup ./mongodb_backup_$(date +%Y%m%d)
```
...
# 删除容器（保留数据卷）
docker rm tradingagents-backend tradingagents-frontend tradingagents-mongodb tradingagents-redis
```
...
docker pull your-registry/tradingagents-frontend:latest
```
...
# 启动 Backend
docker run -d --name tradingagents-backend \
    --env-file .env \
    -p 8000:8000 \
    --link tradingagents-mongodb:mongodb \
    --link tradingagents-redis:redis \
    your-registry/tradingagents-backend:latest

# 启动 Frontend
docker run -d --name tradingagents-frontend \
    -p 3000:80 \
    --link tradingagents-backend:backend \
    your-registry/tradingagents-frontend:latest
```
...
mongo tradingagents --eval "
    print('股票日线数据:', db.stock_daily_quotes.count());
    print('股票基本信息:', db.stock_basic_info.count());
    print('自选股:', db.user_favorites.count());
"
```
...
```bash
# 删除 MongoDB 中的旧数据
mongo tradingagents --eval "db.stock_daily_quotes.deleteMany({data_source: 'tushare'})"
...
```bash
# 查看日志，应该显示：
# ✅ 获取daily历史数据: 300750 167条记录 (前复权 qfq)
```
...
```powershell
# 绿色版
mongorestore --host localhost --port 27017 --db tradingagents --drop C:\Backups\MongoDB\before_upgrade\tradingagents

# Docker 版
docker exec -i tradingagents-mongodb mongorestore --db tradingagents --drop /backup/tradingagents
```
...
```powershell
# 检查端口占用
netstat -ano | findstr "8000 3000 6379 27017"

# 如果有冲突，修改 .env 文件中的端口配置
# 或使用端口冲突检测脚本
.\scripts\portable\check_ports.ps1
```

---

#### Q4: Docker 版升级后镜像拉取失败？
