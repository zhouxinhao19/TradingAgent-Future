...
# 添加: requirepass CHANGE_ME_LOCAL_PASSWORD
...
MONGODB_URL=mongodb://admin:CHANGE_ME_LOCAL_PASSWORD@localhost:27017/tradingagents?authSource=admin
REDIS_URL=redis://:CHANGE_ME_LOCAL_PASSWORD@localhost:6379/0
...
**数据库账号密码说明**：
- **MongoDB**：用户名 `admin`，密码 `CHANGE_ME_LOCAL_PASSWORD`，数据库 `tradingagents`
- **Redis**：密码 `CHANGE_ME_LOCAL_PASSWORD`，无用户名（使用空用户名）
- 这些账号密码配置与 `docker-compose.hub.nginx.yml` 中的配置保持一致

**⚠️ 重要提示**：
- 生产环境请务必修改默认密码
- MongoDB连接字符串中的 `authSource=admin` 参数表示在admin数据库中进行身份验证
- Redis连接字符串中的 `:CHANGE_ME_LOCAL_PASSWORD` 表示空用户名加密码的格式
...
```bash
# 连接MongoDB（使用管理员账户）
mongosh mongodb://admin:CHANGE_ME_LOCAL_PASSWORD@localhost:27017/admin
...
- 导入系统配置数据到 MongoDB
- 创建默认管理员用户（用户名：admin，密码：CHANGE_ME_ADMIN_PASSWORD）
...
```bash
mongosh mongodb://admin:CHANGE_ME_LOCAL_PASSWORD@localhost:27017/tradingagents?authSource=admin
...
**问题**：使用 admin/CHANGE_ME_ADMIN_PASSWORD 无法登录
...
4. **检查密码是否正确**：默认密码是 `CHANGE_ME_ADMIN_PASSWORD`
...
- **文档**: 项目 `docs/` 目录下的详细文档
- **微信公众号**: 搜索"TradingAgent-Future"关注获取最新资讯
