...
- **新闻分析师**：专业的财经新闻分析智能体，能够分析新闻对期货价格的潜在影响
- **统一新闻接口**：自动识别期货品种类型并选择合适的新闻源的统一接口
- **东方财富新闻**：通过AKShare获取东方财富网的品种新闻，为A股和港股提供专业的中文财经新闻
...
系统已集成AKShare库，用于获取东方财富网的品种新闻。AKShare是一个优秀的Python金融数据接口库，无需API密钥，但需要确保已正确安装：
...
新闻分析师是一个专业的财经新闻分析智能体，能够分析新闻对期货价格的潜在影响：
...
2. **使用统一接口处理多市场**：当需要分析不同市场（A股、港股、美股）的期货品种时，使用 `get_stock_news_unified` 方法可以自动选择合适的新闻源。

3. **利用东方财富新闻**：对于A股和港股分析，系统会自动获取东方财富网的专业财经新闻，提供更准确的中文市场信息。

4. **结合全球宏观新闻**：使用 `get_global_news_openai` 获取全球宏观经济新闻，与期货品种特定新闻结合分析，获得更全面的市场视角。
...
新闻分析系统提供了全面的新闻获取和分析功能，能够从多个数据源获取实时新闻，并进行专业的分析和评估。统一的新闻获取接口使得系统能够自动识别期货品种类型并选择合适的新闻源，提供格式化的新闻分析报告。新闻分析师能够分析新闻对期货价格的潜在影响，提供量化的交易建议和价格影响评估。
