...
DeepSeek V3 是TradingAgent-Future v0.1.7新集成的成本优化AI模型，专为中文金融场景设计。相比GPT-4，DeepSeek V3在保持优秀分析质量的同时，成本降低90%以上，是进行期货分析的理想选择。
