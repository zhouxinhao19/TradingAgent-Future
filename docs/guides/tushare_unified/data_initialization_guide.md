...
### 核心数据类型
1. **期货品种基础信息** - 品种代码、名称、行业、市场等基础数据
2. **历史行情数据** - 指定时间范围的历史价格数据（支持日线、周线、月线）
3. **财务数据** - 财报、指标等财务信息
4. **实时行情数据** - 最新的期货价格和交易数据

### 预期成果
- ✅ 完整的期货品种基础信息库（5000+只期货品种）
- ✅ 指定时间范围的历史数据（默认1年）
- ✅ 最新的财务数据和行情数据
- ✅ 标准化的数据格式和索引
...
| 数据类型 | 数量级 | 预计耗时 | 说明 |
|---------|--------|----------|------|
| 基础信息 | 5000+期货品种 | 5-10分钟 | 取决于网络和API限制 |
...
| 数据类型 | 标识符 | 说明 |
|---------|--------|------|
| 期货品种基础信息 | `basic_info` | 品种代码、名称、行业等基础信息 |
...
全历史数据同步功能允许您获取期货品种从上市以来的完整历史数据（从1990年至今），而不仅仅是最近几年的数据。
...
2025-09-29 09:05:00 | INFO | ✅ 基础信息初始化完成: 5,436只期货品种
