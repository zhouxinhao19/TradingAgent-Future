...
| 数据类型 | 覆盖范围 | 更新频率 | 数据源 |
|---------|----------|----------|--------|
| **期货品种基础信息** | 全市场国内期货 | 每日 | AKShare |
| **实时行情** | 全市场国内期货 | 交易时间10分钟 | AKShare |
...
### 性能指标
- **基础信息同步**: ~5000期货品种，5-10分钟
- **实时行情同步**: ~5000期货品种，3-5分钟
- **历史数据同步**: 取决于时间范围和网络状况
- **内存使用**: 优化的流式处理，内存占用低
...
1. **检查数据库状态** - 评估现有数据情况
2. **初始化期货品种基础信息** - 获取全市场期货品种列表和基础信息
