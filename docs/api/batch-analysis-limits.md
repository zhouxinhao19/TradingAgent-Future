...
假设用户提交了5个品种代码：`["000001", "600519", "600036", "000002", "600000"]`
...
如果提交了超过10个品种代码，会收到以下错误：

```json
{
  "success": false,
  "message": "批量分析最多支持 10 品种票，当前提交了 15 个"
}
```

### 空列表

如果提交了空的品种代码列表，会收到以下错误：
...
symbols: Optional[List[str]] = Field(None, min_items=1, max_items=20, description="品种代码列表（最多20个）")
...
**A**: 目前不支持取消排队中的任务。一旦任务被创建，就会按顺序执行。建议在提交前仔细确认品种代码列表。

### Q4: 如果提交了10品种票，可以再提交10个吗？
