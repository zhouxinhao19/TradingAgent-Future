...
def demo_basic_usage():
    ...
    try:
        ...
        if not search_results.empty:
            print(f"搜索'银行'找到 {len(search_results)} 只股票")
            print("\n前5个结果:")
            for idx, row in search_results.head(5).iterrows():
                print(f"  {row['symbol']} - {row['name']} ({row.get('industry', '未知')})")
...
def demo_batch_operations():
    ...
    try:
        ...
        print(f"📊 批量获取 {len(symbols)} 只股票信息")
        print("-" * 40)
        ...
        for i, symbol in enumerate(symbols, 1):
            try:
                ...
                if stock_info:
                    print(f"{i}. {symbol} - {stock_info.get('name')} ({stock_info.get('industry', '未知')})")
                else:
...
def demo_cache_performance():
    ...
    try:
        ...
        if not data1.empty:
            print(f"✅ 获取成功: {len(data1)}条数据，耗时: {time1:.2f}秒")
        else:
            print("❌ 获取失败")
            return
        ...
        if not data2.empty:
            print(f"✅ 获取成功: {len(data2)}条数据，耗时: {time2:.2f}秒")
...
def check_environment():
    ...
    if token:
        print(f"✅ API Token: 已设置 ({len(token)}字符)")
    else:
        print("❌ API Token: 未设置")
...
def main():
    ...
    print("\n🎉 Tushare演示完成！")
    print("\n📚 更多信息:")
    print("   - 文档: docs/data/tushare-integration.md")
    print("   - 测试: tests/test_tushare_integration.py")
    print("   - 配置: config/tushare_config.example.env")
